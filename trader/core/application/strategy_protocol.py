...
class ValidationResult:
    ...
    @classmethod
    def invalid(
        cls, errors: Sequence[ValidationError], metadata: Optional[Dict[str, Any]] = None
    ) -> ValidationResult:
        """创建无效验证结果"""
        return cls(status=ValidationStatus.INVALID, errors=errors, metadata=metadata or {})

    @classmethod
    def with_warnings(
        cls, warnings: Sequence[str], metadata: Optional[Dict[str, Any]] = None
    ) -> ValidationResult:
        """创建带警告的验证结果"""
        return cls(status=ValidationStatus.WARNING, warnings=warnings, metadata=metadata or {})

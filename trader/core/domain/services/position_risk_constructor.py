...
class PositionRiskConstructorConfig:
    ...
    # Regime 风险折扣
    regime_discounts: Dict[MarketRegime, Decimal] = field(
...
class PositionRiskConstructor:
    ...
    def check_per_symbol_exposure(
        ...
        if (
            total_portfolio_value is not None
            and total_portfolio_value > 0
            and self._config.max_position_size_percent > 0
        ):
            max_position_pct_value = (
                total_portfolio_value * self._config.max_position_size_percent / Decimal("100")
            )
    ...
    def check_total_exposure(
        ...
        total_current = sum(
            (abs(p.market_value) for p in positions if p.quantity != 0), Decimal("0")
        )
    ...
    def check_cooldown(
        ...
        if not self._config.cooldown_enabled:
            return CooldownResult(allowed=True, last_trade_time=None, message="冷却期检查已禁用")
        ...
        if self._cooldown_tracker is None:
            return CooldownResult(
                allowed=True, last_trade_time=None, message="冷却期追踪器未配置，跳过检查"
            )
        ...
        if last_trade is None:
            return CooldownResult(
                allowed=True, last_trade_time=None, message=f"币种 {symbol} 无交易历史"
            )

...
class EscapeTimeResult:
    ...
    @classmethod
    def blocked(cls, blocking_factors: List[str], estimated_seconds: int = 0) -> "EscapeTimeResult":
        """创建被阻止的结果"""
        return cls(
    ...
    def can_escape_result(
        cls, estimated_seconds: int, max_slippage_bps: int, escape_path: List[DepthLevel]
    ) -> "EscapeTimeResult":
        """创建可以平仓的结果"""
        return cls(
            estimated_seconds=estimated_seconds,
...
class EscapeTimeSimulator:
    ...
    def estimate_escape_time(
        ...
        if position.quantity == 0:
            return EscapeTimeResult.can_escape_result(
                estimated_seconds=0, max_slippage_bps=0, escape_path=[]
            )
        ...
        estimated_seconds = self._estimate_time(
            position=position, depth_result=depth_result, regime=regime
        )
    ...
    def _check_killswitch(self, scope: str) -> Optional[EscapeTimeResult]:
        ...
        if level >= KillSwitchLevel.L2_CANCEL_ALL_AND_HALT:
            level_name = (
                "L2_CANCEL_ALL_AND_HALT"
                if level == KillSwitchLevel.L2_CANCEL_ALL_AND_HALT
                else "L3_LIQUIDATE_AND_DISCONNECT"
            )
            return EscapeTimeResult.blocked(blocking_factors=[level_name], estimated_seconds=0)

        return None

    def _check_cooldown(
        self, symbol: str, current_time: Optional[datetime]
        ...
        if remaining.total_seconds() > 0:
            return EscapeTimeResult.blocked(
                blocking_factors=["IN_COOLDOWN"], estimated_seconds=int(remaining.total_seconds())
            )
    ...
    def _simulate_depth_escape(
        ...
        if not levels:
            return _DepthSimResult(max_slippage_bps=0, escape_path=[], insufficient_depth=True)

        # 获取 regime 折扣因子
        regime_discount = self._risk_config.regime_discounts.get(regime, Decimal("1.0"))
        ...
        for idx, level in enumerate(levels):
            ...
            depth_level = DepthLevel(
                price=level.price, cumulative_quantity=new_cumulative, level_index=idx
            )

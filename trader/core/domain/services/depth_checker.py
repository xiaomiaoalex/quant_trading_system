...
from trader.core.domain.models.orderbook import DepthCheckResult, OrderBook
...
class DepthChecker:
    ...
    def check_depth(
        self, orderbook: OrderBook, target_qty: Decimal, side: OrderSide
    ) -> DepthCheckResult:
        """
        检查订单簿深度
        ...
        if accumulated_qty < target_qty:
            return DepthCheckResult.reject_insufficient_depth(
                available_qty=float(accumulated_qty), required_qty=float(target_qty)
            )
        ...
        return DepthCheckResult.pass_result(
            slippage_bps=slippage_bps, available_qty=float(accumulated_qty)
        )

    def check_signal_depth(self, orderbook: OrderBook, signal: Signal) -> DepthCheckResult:
        """
        检查交易信号的订单簿深度
        ...
        return self.check_depth(orderbook=orderbook, target_qty=signal.quantity, side=side)
...
class DepthCheckPreTradePlugin:
    ...
    def __init__(self, market_data: MarketDataPort, config: Optional[DepthCheckerConfig] = None):
        self._market_data = market_data
        self._config = config or DepthCheckerConfig()
        self._checker = DepthChecker(market_data=market_data, config=self._config)

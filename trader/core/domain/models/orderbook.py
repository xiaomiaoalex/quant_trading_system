...
class DepthCheckResult:
    ...
    @classmethod
    def reject_insufficient_depth(
        cls, available_qty: float, required_qty: float
    ) -> "DepthCheckResult":
        """创建深度不足拒绝结果"""
        return cls(
            ok=False,
    ...
    @classmethod
    def reject_excessive_slippage(
        cls, slippage_bps: float, max_slippage_bps: float, available_qty: float
    ) -> "DepthCheckResult":
        """创建滑点超限拒绝结果"""
        return cls(
            ok=False,

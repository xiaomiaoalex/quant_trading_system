...
from trader.services.strategy_event_service import (
    ...
    StrategyEventType,
    get_strategy_event_service,
)


def _create_event_callback():
    """创建事件发布回调函数"""
    service = get_strategy_event_service()
    
    async def event_publisher(strategy_id: str, event_type: str, payload: dict) -> None:
        event = StrategyEvent(
            strategy_id=strategy_id,
            event_type=StrategyEventType(event_type),
            payload=payload,
        )
...
async def get_strategy_events(
    strategy_id: str,
    event_type: Optional[str] = Query(None, description="Filter by event type (e.g., strategy.signal)"),
    limit: int = Query(100, ge=1, le=1000, description="Max results"),
):
    """
    Get strategy runtime events (signals, orders, errors).
    
    Returns a list of events for the specified strategy.
    """
    service = get_strategy_event_service()
...
async def get_strategy_signals(
    ...
    """
    Get recent signal events for a strategy.
    """
    service = get_strategy_event_service()
    return await service.get_recent_signals(strategy_id=strategy_id, limit=limit)
...
async def get_strategy_errors(
    ...
    """
    Get recent error events for a strategy.
    """
    service = get_strategy_event_service()
    return await service.get_recent_errors(strategy_id=strategy_id, limit=limit)

...
class BacktestEngine:
            ...
            klines.append(
                {
                    ...
                    "close_time": int(
                        (current + timedelta(minutes=interval_minutes)).timestamp() * 1000
                    ),
    ...
    def _calculate_metrics(
        ...
        # 盈亏比
        avg_win = (
            gross_profit / Decimal(str(winning_trades)) if winning_trades > 0 else Decimal("0")
        )
        avg_loss = (
            gross_loss / Decimal(str(total_trades - winning_trades))
        ...
        if len(self._equity_curve) > 1:
            ...
            for i in range(1, len(self._equity_curve)):
                ret = (self._equity_curve[i] - self._equity_curve[i - 1]) / self._equity_curve[
                    i - 1
                ]
...
class LiveEvaluator:
    ...
    def _check_deviation(
        ...
        # 检查胜率偏离
        if b_metrics.win_rate > 0 and metrics.win_rate < b_metrics.win_rate * (
            1 - self._config.alert_threshold_winrate_drop
        ):
            result.add_alert(
                AnomalyAlert(

...
class PerformanceBenchmark:
    ...
    def test_1year_backtest(self) -> BenchmarkResult:
        ...
        try:
            ...
            if passed:
                logger.info(
                    f"1-year backtest passed: {duration:.2f}s (target: {self._targets.one_year_backtest_max}s)"
                )
            else:
                logger.warning(
                    f"1-year backtest FAILED: {duration:.2f}s (target: {self._targets.one_year_backtest_max}s)"
                )
    ...
    def test_5year_backtest(self) -> BenchmarkResult:
        ...
        try:
            ...
            if passed:
                logger.info(
                    f"5-year backtest passed: {duration:.2f}s (target: {self._targets.five_year_backtest_max}s)"
                )
            else:
                logger.warning(
                    f"5-year backtest FAILED: {duration:.2f}s (target: {self._targets.five_year_backtest_max}s)"
                )
    ...
    def test_memory_usage(self) -> BenchmarkResult:
        ...
        try:
            ...
            if passed:
                logger.info(
                    f"Memory usage passed: {peak_mb:.2f}MB (target: {self._targets.memory_max/(1024*1024):.0f}MB)"
                )
            else:
                logger.warning(
                    f"Memory usage FAILED: {peak_mb:.2f}MB (target: {self._targets.memory_max/(1024*1024):.0f}MB)"
                )
    ...
    def test_throughput(self) -> BenchmarkResult:
        ...
        try:
            ...
            if passed:
                logger.info(
                    f"Throughput passed: {bars_per_sec:.0f} bars/s (target: {self._targets.min_bars_per_second:.0f})"
                )
            else:
                logger.warning(
                    f"Throughput FAILED: {bars_per_sec:.0f} bars/s (target: {self._targets.min_bars_per_second:.0f})"
                )

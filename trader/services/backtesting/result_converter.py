...
class QuantConnectTrade:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.entry_time, (int, float)):
            object.__setattr__(
                self, "entry_time", datetime.fromtimestamp(self.entry_time, tz=timezone.utc)
            )
        ...
        if isinstance(self.exit_time, (int, float)) and self.exit_time is not None:
            object.__setattr__(
                self, "exit_time", datetime.fromtimestamp(self.exit_time, tz=timezone.utc)
            )
...
class QuantConnectStatistics:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.max_drawdown_percent, (int, float)):
            object.__setattr__(
                self, "max_drawdown_percent", Decimal(str(self.max_drawdown_percent))
            )
        ...
        if isinstance(self.average_trade_duration, (int, float)):
            object.__setattr__(
                self, "average_trade_duration", Decimal(str(self.average_trade_duration))
            )
...
class BacktestResultConverter:
    ...
    def convert(
        ...
        try:
            ...
            equity_points, equity_warnings = self._convert_equity_curve(
                lean_result.get("EquityCurve", [])
            )
            ...
            final_capital = (
                stats_data.final_capital if stats_data else (initial_capital + total_profit)
            )
            ...
            if losing_trades > 0:
                avg_win = (
                    sum(t.pnl for t in trade_list if t.pnl > 0) / Decimal(str(winning_trades))
                    if winning_trades > 0
                    else Decimal("0")
                )
                avg_loss = abs(
                    sum(t.pnl for t in trade_list if t.pnl < 0) / Decimal(str(losing_trades))
                )
                if avg_loss > 0:
                    profit_factor = avg_win / avg_loss

            max_dd = (
                stats_data.max_drawdown
                if stats_data
                else self._calculate_max_drawdown(equity_points)
            )
            ...
            sharpe = (
                ...
                if stats_data
                else self._calculate_sharpe_ratio(equity_points, initial_capital)
            )
            sortino = (
                stats_data.sortino_ratio
                if stats_data
                else self._calculate_sortino_ratio(equity_points, initial_capital)
            )
            calmar = (
                stats_data.calmar_ratio
                if stats_data
                else self._calculate_calmar_ratio(equity_points, initial_capital)
            )
            var_95 = stats_data.var_95 if stats_data else self._calculate_var(equity_points, 0.95)

            avg_duration = (
                stats_data.average_trade_duration
                if stats_data
                else self._calculate_avg_trade_duration(trade_list)
            )
            ...
            if config is None:
                config = BacktestConfig(
                    start_date=(
                        equity_points[0].timestamp if equity_points else datetime.now(timezone.utc)
                    ),
                    end_date=(
                        equity_points[-1].timestamp if equity_points else datetime.now(timezone.utc)
                    ),
    ...
    ) -> tuple[QuantConnectStatistics, List[str]]:
    ...
    def _convert_equity_curve(
        self, curve: List[Dict[str, Any]]
    ) -> tuple[List[EquityPoint], List[str]]:
        ...
        for i, point in enumerate(curve):
            try:
                if isinstance(point, dict):
                    ...
                    points.append(
                        EquityPoint(
                            timestamp=datetime.fromtimestamp(point, tz=timezone.utc),
                            equity=Decimal("0"),
                        )
                    )
                else:
    ...
    ) -> tuple[List[QuantConnectTrade], List[str]]:
            ...
            try:
                if isinstance(trade, dict):
                    converted_trade = QuantConnectTrade(
                        ...
                        entry_price=Decimal(
                            str(trade.get("EntryPrice", trade.get("entry_price", 0)))
                        ),
                        exit_price=(
                            Decimal(str(trade.get("ExitPrice", trade.get("exit_price", 0))))
                            if trade.get("ExitPrice", trade.get("exit_price"))
                            else Decimal("0")
                        ),
                        pnl=Decimal(str(trade.get("PnL", trade.get("pnl", 0)))),
                        pnl_percent=Decimal(
                            str(trade.get("PnLPercent", trade.get("pnl_percent", 0)))
                        ),
                    )
    ...
    def _calculate_calmar_ratio(
        ...
        years = (
            Decimal(str(len(equity_curve) / self._trading_days))
            if self._trading_days > 0
            else Decimal("1")
        )
        annualized_return = (
            (end_equity / start_equity) ** (Decimal("1") / years) - 1

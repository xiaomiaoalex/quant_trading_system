...
class DirectionAwareSlippage:
    ...
    def calculate_rate(
        self, execution_price: Decimal, open_price: Decimal, side: OrderSide
    ) -> Decimal:
        """
        计算实际滑点率
...
class NextBarOpenExecutor:
    ...
    def execute_pending(
        ...
        for order_id, order in self._pending_orders.items():
            ...
            if self._config.enable_slippage and exit_reason in (
                ExitReason.SIGNAL,
                ExitReason.TIMEOUT,
            ):
    ...
    def check_exits(self, bar: OHLCV) -> List[ExecutionResult]:
        ...
        for symbol, position in self._positions.items():
            ...
            if exit_reason is not None:
                ...
                ), "trigger_price must not be None when exit_reason is set"

...
class BacktestVisualizer:
    ...
    def plot_equity_curve(
        ...
        timestamps = self._parse_timestamps(
            [
                p.get("timestamp") if isinstance(p, dict) else getattr(p, "timestamp", None)
                for p in equity_curve
            ]
        )
        equities = self._parse_equities(
            [
                p.get("equity") if isinstance(p, dict) else getattr(p, "equity", 0)
                for p in equity_curve
            ]
        )
        ...
        ax.plot(
            timestamps,
            equities,
            color=self.config.color_scheme["equity"],
            linewidth=2,
            label="Strategy",
        )
        ...
        if benchmark:
            ...
            if bench_ts and bench_eq:
                ax.plot(
                    bench_ts,
                    bench_eq,
                    color=self.config.color_scheme["benchmark"],
                    linewidth=1.5,
                    linestyle="--",
                    label="Buy & Hold",
                )
        ...
        if show_cagr and hasattr(report, "returns"):
            cagr = (
                getattr(report.returns, "annual_return", None)
                or getattr(report, "result", None)
                and getattr(report.result, "total_return", None)
            )
            if cagr is not None:
    ...
    def plot_drawdown(
        ...
        ax.fill_between(
            timestamps, drawdowns, 0, color=self.config.color_scheme["drawdown"], alpha=0.5
        )
    ...
    def plot_trade_markers(
        ...
        if trades:
            for trade in trades:
                ...
                entry_price = (
                    ...
                    if isinstance(trade, dict)
                    else getattr(trade, "entry_price", 0)
                )
                exit_price = (
                    trade.get("exit_price", 0)
                    if isinstance(trade, dict)
                    else getattr(trade, "exit_price", 0)
                )
                direction = (
                    trade.get("direction", "UNKNOWN")
                    if isinstance(trade, dict)
                    else getattr(trade, "direction", "UNKNOWN")
                )
                ...
                if exit_time:
                    color = (
                        self.config.color_scheme["profit"]
                        if pnl > 0
                        else self.config.color_scheme["loss"]
                    )
        ...
        exit_profit = mpatches.Patch(
            color=self.config.color_scheme["profit"], label="Exit (Profit)"
        )
        exit_loss = mpatches.Patch(color=self.config.color_scheme["loss"], label="Exit (Loss)")
        ax.legend(
            handles=[entry_patch, exit_profit, exit_loss],
            loc="upper left",
    ...
    def plot_returns_distribution(
        ...
        ax_hist.axvline(
            x=sum(returns) / len(returns),
            color="orange",
            linestyle="-",
            linewidth=1.5,
            label=f"Mean: {sum(returns)/len(returns):.4f}",
        )
    ...
    def plot_combined(
        ...
        fig = plt.figure(
            figsize=(self.config.figsize[0], self.config.figsize[1] * 2), dpi=self.config.dpi
        )
        ...
        if monthly_returns:
            ...
            colors = [
                self.config.color_scheme["profit"] if v >= 0 else self.config.color_scheme["loss"]
                for v in values
            ]

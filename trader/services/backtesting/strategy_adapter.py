...
from typing import Any, Callable, Dict, List, Optional, Sequence, runtime_checkable
...
from trader.services.backtesting.ports import FrameworkType, StrategyAdapterPort
...
class IndicatorMapper:
    ...
    def resolve_indicator(self, name: str) -> IndicatorType:
        ...
        if normalized not in self._indicator_registry:
            raise ValueError(
                f"Unknown indicator: {name}. Supported: {list(self._indicator_registry.keys())}"
            )
    ...
    def _generate_indicator_initialization(self) -> str:
        ...
        for config in self._config.indicators:
            ...
            if config.indicator_type == IndicatorType.MACD:
                ...
                lines.append(
                    f"            {var_name} = MACD({config.symbol}, {fast}, {slow}, {signal}, MovingAverageType.Exponential);"
                )
                ...
                lines.append(
                    f"            {var_name} = BB({config.symbol}, {period}, {std}, MovingAverageType.Simple);"
                )
            else:
                lines.append(
                    f"            {var_name} = {self._indicator_mapper.get_lean_indicator_code(config)};"
                )
    ...
    def convert_to_framework_format(
        ...
        if not isinstance(strategy, StrategyPlugin):
            raise ConversionError(
                f"Strategy must implement StrategyPlugin protocol, got {type(strategy)}"
            )

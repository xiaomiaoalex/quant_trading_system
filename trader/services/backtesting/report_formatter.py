...
class ReportFormatter:
    ...
    def format_with_benchmark(
        ...
        benchmark = self._calculate_benchmark_comparison(backtest_result, benchmark_result, config)
    ...
    def calculate_buy_and_hold(
        ...
        first_price = (
            equity_curve[0].get("equity", initial_capital)
            if isinstance(equity_curve[0], dict)
            else equity_curve[0].equity
        )
        last_price = (
            equity_curve[-1].get("equity", initial_capital)
            if isinstance(equity_curve[-1], dict)
            else equity_curve[-1].equity
        )

        total_return = (
            ((last_price - first_price) / first_price) * 100 if first_price > 0 else Decimal("0")
        )

        # Calculate annual return
        if len(equity_curve) >= 2:
            first_ts = (
                equity_curve[0].get("timestamp", datetime.now(timezone.utc))
                if isinstance(equity_curve[0], dict)
                else equity_curve[0].timestamp
            )
            last_ts = (
                equity_curve[-1].get("timestamp", datetime.now(timezone.utc))
                if isinstance(equity_curve[-1], dict)
                else equity_curve[-1].timestamp
            )
            ...
            annual_return = (
                Decimal(str(float(last_price / first_price) ** (1 / years) - 1)) * 100
                if years > 0
                else Decimal("0")
            )
        else:
    ...
    def _calculate_return_metrics(
        ...
        final_capital = (
            ...
            if hasattr(result, "final_capital") and result.final_capital
        ...
        annual_return = self._calculate_annual_return(
            result, equity_curve, initial_capital, final_capital
        )
    ...
    def _calculate_annual_return(
        ...
        annual_return = (
            Decimal(str(float(final_capital / initial_capital) ** (1 / years) - 1)) * 100
        )
    ...
    def _calculate_monthly_returns(self, equity_curve: List[Any]) -> List[Decimal]:
        ...
        for i, month_key in enumerate(sorted_months):
            ...
            first_equity = (
                month_points[0].get("equity", Decimal("0"))
                if isinstance(month_points[0], dict)
                else month_points[0].equity
            )
            last_equity = (
                month_points[-1].get("equity", Decimal("0"))
                if isinstance(month_points[-1], dict)
                else month_points[-1].equity
            )
    ...
    def _calculate_equity_drawdown(
        ...
        for point in equity_curve:
            equity = (
                point.get("equity", initial_capital) if isinstance(point, dict) else point.equity
            )
    ...
    def _calculate_period_returns(self, equity_curve: List[Any]) -> List[Decimal]:
        ...
        for i in range(1, len(equity_curve)):
            prev_equity = (
                equity_curve[i - 1].get("equity", Decimal("0"))
                if isinstance(equity_curve[i - 1], dict)
                else equity_curve[i - 1].equity
            )
            curr_equity = (
                equity_curve[i].get("equity", Decimal("0"))
                if isinstance(equity_curve[i], dict)
                else equity_curve[i].equity
            )
    ...
    def _calculate_trade_statistics(
        ...
        for trade in trades:
            pnl = (
                ...
                else getattr(trade, "pnl", Decimal("0"))
            )
            ...
            # Duration
            entry_time = (
                trade.get("entry_time", datetime.now(timezone.utc))
                if isinstance(trade, dict)
                else getattr(trade, "entry_time", None)
            )
            exit_time = (
                trade.get("exit_time", datetime.now(timezone.utc))
                if isinstance(trade, dict)
                else getattr(trade, "exit_time", None)
            )
    ...
    def _calculate_benchmark_comparison(
        ...
        # Strategy returns
        strategy_return = (
            backtest_result.result.total_return
            if hasattr(backtest_result, "result")
            else Decimal("0")
        )
    ...
    def _create_meta_info(
        ...
        start_date = (
            config.start_date
            if config and hasattr(config, "start_date")
            else datetime.now(timezone.utc)
        )
        end_date = (
            config.end_date
            if config and hasattr(config, "end_date")
            else datetime.now(timezone.utc)
        )

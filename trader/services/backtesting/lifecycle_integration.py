...
if TYPE_CHECKING:
    from trader.services.backtesting.ports import BacktestConfig, BacktestReport, BacktestResult
    ...
    def compare(
        ...
        for metric in metrics:
            ...
            comparison["rankings"][metric] = [
                {"strategy": s, "value": v, "rank": i + 1} for i, (s, v) in enumerate(ranking)
            ]
        ...
        comparison["summary"] = {
            ...
            "best_strategy": (
                max(overall_scores.items(), key=lambda x: x[1])[0] if overall_scores else None
            ),
        }

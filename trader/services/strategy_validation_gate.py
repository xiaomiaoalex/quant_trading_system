...
class StrategyValidationGate:
    ...
    async def validate(
        ...
        # Layer 3: 样本外验证
        layer3_passed, layer3_reason = await self.validate_layer3(walkforward_result, kfold_result)
    ...
    def _generate_recommendations(
        ...
        if 2 in report.failed_layers:
            recommendations.append("Layer 2: 检查回测配置，确保无前瞻偏差、方向感知滑点、止盈止损")

        if 3 in report.failed_layers:
            recommendations.append("Layer 3: 重新设计策略参数，减少过拟合风险")

        if 4 in report.failed_layers:
            recommendations.append("Layer 4: 策略边缘较薄，考虑收紧成本假设或调整策略")

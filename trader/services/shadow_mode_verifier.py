...
class ShadowModeVerifier:
    ...
    async def verify(
        ...
        if len(backtest_signals) > 0 and len(shadow_signals) > 0:
            backtest_rate = (
                len(backtest_signals) / report.total_signals if report.total_signals > 0 else 0
            )
            shadow_rate = (
                len(shadow_signals) / report.total_signals if report.total_signals > 0 else 0
            )
        ...
        if len(backtest_signals) > 0:
            report.risk_block_rate_diff = abs(
                backtest_blocked / len(backtest_signals) - shadow_blocked / len(shadow_signals)
                if len(shadow_signals) > 0
                else 0
            )
    ...
    def _match_sizing_pairs(
        ...
        shadow_map = {(s.symbol, s.timestamp): s.sizing for s in shadow_signals}
    ...
    def _evaluate_health(self, report: ShadowDeviationReport) -> bool:
        ...
        if report.sizing_max_diff > self._sizing_threshold * 2:
            alerts.append(f"Sizing 最大偏差 {report.sizing_max_diff:.1%} 异常高")
        ...
        if report.risk_block_rate_diff > 0.5:
            alerts.append(f"风控拦截率差异 {report.risk_block_rate_diff:.1%} 超过 50%")

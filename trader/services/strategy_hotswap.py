...
class SwapResult:
    ...
    position_mappings: Dict[str, Tuple[Position, Optional[str]]] = field(
        default_factory=dict
    )  # symbol -> (position, new_strategy_tag)
...
class PositionProviderPort(Protocol):
    ...
    async def migrate_position(self, symbol: str, from_strategy: str, to_strategy: str) -> bool:
        """迁移持仓到新策略"""
        ...
...
class StrategyLoader:
    ...
    async def load_from_module(
        ...
        try:
            ...
            if not isinstance(plugin, StrategyPlugin):
                raise TypeError(f"模块 {module_path} 返回的对象未实现 StrategyPlugin 协议")
            ...
            logger.info(
                f"策略加载成功（从模块）: {strategy_id} v{getattr(plugin, 'version', 'unknown')}"
            )
            return plugin
    ...
    async def load_from_code(
        ...
        if signature:
            ...
            logger.warning(f"代码签名未提供，建议为AI生成的代码提供签名: {strategy_id}")
        ...
        try:
            ...
            # 验证协议
            if not isinstance(plugin, StrategyPlugin):
                raise TypeError(f"策略未实现 StrategyPlugin 协议")
...
class VersionManager:
    ...
    def __init__(
        ...
        self._stored_strategies: Dict[str, StoredStrategy] = (
            {}
        )  # version_id string -> stored strategy

    def _make_version_id_str(self, version_id: VersionId) -> str:
        """生成版本ID字符串"""
        return (
            f"{version_id.strategy_id}@{version_id.version}@{int(version_id.timestamp.timestamp())}"
        )
...
class StrategyHotSwapper:
    ...
    async def _transition_state(self, new_state: SwapState, new_phase: SwapPhase) -> None:
        ...
        logger.info(
            f"热插拔状态转换: {old_state.value}.{old_phase.value} -> {new_state.value}.{new_phase.value}"
        )
    ...
    async def swap(
        ...
        async with lock:
            ...
            old_strategy_id = (
                getattr(old_strategy, "strategy_id", getattr(old_strategy, "name", ""))
                if old_strategy
                else ""
            )
            ...
            try:
                ...
                await self._transition_state(
                    SwapState.VALIDATING, SwapPhase.VALIDATING_RESOURCE_LIMITS
                )
                ...
                if limits and hasattr(limits, "max_orders_per_minute"):
                    if limits.max_orders_per_minute <= 0:
                        ...
                        self._current_swap.state = SwapState.ERROR
                        await self._transition_state(
                            SwapState.ERROR, SwapPhase.VALIDATING_RESOURCE_LIMITS
                        )
                        return self._current_swap
                ...
                # ========== PREPARING: 持仓迁移 ==========
                await self._transition_state(
                    SwapState.PREPARING, SwapPhase.PREPARING_MIGRATE_POSITIONS
                )
                ...
                if old_strategy and hasattr(old_strategy, "shutdown"):
                    try:
                        await asyncio.wait_for(old_strategy.shutdown(), timeout=5.0)
                ...
                if hasattr(new_strategy, "initialize"):
                    try:
                        await asyncio.wait_for(new_strategy.initialize({}), timeout=10.0)
                    except asyncio.TimeoutError:
                        error = await self._set_error(
                ...
                await self._transition_state(
                    SwapState.SWITCHING, SwapPhase.SWITCHING_UPDATE_REGISTRY
                )
                ...
                logger.info(
                    f"策略热插拔成功: {old_strategy_id} -> {strategy_id}, 耗时: {self._current_swap.duration_ms:.2f}ms"
                )
                return self._current_swap
    ...
    async def _rollback(self, strategy_id: str) -> None:
        ...
        if self._new_strategy and hasattr(self._new_strategy, "shutdown"):
            try:
                await asyncio.wait_for(self._new_strategy.shutdown(), timeout=5.0)
            except Exception as e:

...
class AllocationManagementService:
    ...
    async def upsert_profile_async(
        self, deployment_id: str, request: StrategyAllocationProfileUpdateRequest
    ) -> StrategyAllocationProfile:
        old_profile = self._storage.get_allocation_profile(deployment_id)
        profile_data = await self._build_profile_data_async(deployment_id, request)
        profile = self._storage.upsert_allocation_profile(deployment_id, profile_data)
        self._append_profile_update_event(
            deployment_id=deployment_id,
            old_profile=old_profile,
    ...
    async def _build_profile_data_async(
        self, deployment_id: str, request: StrategyAllocationProfileUpdateRequest
    ) -> dict[str, Any]:
        data = request.model_dump()
        allocation_mode = request.allocation_mode
        ...
        if allocation_mode == "ABSOLUTE_NOTIONAL":
            ...
            if request.target_weight is None:
                raise ValueError("target_weight is required for PERCENT_OF_NAV allocation")
            basis_nav = await self._resolve_basis_nav_async(
                deployment_id=deployment_id,
                nav_source=request.nav_source,
                manual_nav=request.manual_nav,
            )
    ...
    async def _resolve_basis_nav_async(
        self,
        *,
        deployment_id: str,
        nav_source: str,
        ...
        if nav_source != "paper_nav":
            return self._resolve_basis_nav(
                deployment_id=deployment_id,
                nav_source=nav_source,
                manual_nav=manual_nav,
            )

        value = self._resolve_paper_nav_from_memory(deployment_id)
        if value is not None:
            return value

        deployment = self._storage.get_deployment(deployment_id)
        strategy_id = deployment.get("strategy_id") if deployment is not None else None
        if strategy_id and strategy_id != deployment_id:
            value = self._resolve_paper_nav_from_memory(str(strategy_id))
            if value is not None:
                return value

        from trader.storage.nav_store import get_nav_series_pg

        for lookup_id in (deployment_id, str(strategy_id) if strategy_id else None):
            if not lookup_id:
                continue
            series = await get_nav_series_pg(lookup_id, limit=1)
            if series:
                value = _positive_float(series[-1].get("equity"))
                if value is not None:
                    return value
        return None

    def _resolve_paper_nav_from_memory(self, deployment_id: str) -> float | None:
        series = self._storage.get_nav_series(deployment_id)
        if not series:
            return None
        return _positive_float(series[-1].get("equity"))

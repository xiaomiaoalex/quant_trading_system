...
class TestEventWriting:
    ...
    async def test_write_failure(self, mock_event_store):
        """写入失败"""
        mock_event_store.append_domain_event = AsyncMock(side_effect=Exception("Write failed"))
    ...
    async def test_write_with_none_latest_seq(self):
        ...
        assert call_args.kwargs.get("seq") == 0 or (
            len(call_args.args) >= 3 and call_args.args[2] == 0
        )
    ...
    async def test_write_with_existing_latest_seq(self):
        ...
        call_args = mock_store.append_domain_event.call_args
        assert call_args is not None
        assert call_args.kwargs.get("seq") == 100 or (
...
class TestWsSourceParsing:
    ...
    def test_parse_unknown_type_continues(self):
        """未知类型的消息应该被跳过而不是退出循环"""
        unknown_msg = {"type": "UNKNOWN", "someField": "value"}
...
class TestWsToHtmlFailover:
    ...
    async def test_fetch_announcements_uses_ws_when_available(self):
        ...
        # Mock WS source 返回数据
        mock_ws = MagicMock()
        mock_ws.fetch_initial = AsyncMock(
            return_value=[
                RawAnnouncement(
                    announcement_id="789",

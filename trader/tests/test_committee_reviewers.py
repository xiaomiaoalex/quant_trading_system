...
class TestOrthogonalityAgent:
    ...
    def test_text_similarity_calculation(self):
        """测试文本相似度计算"""
        # 相似文本
        score1 = self.agent._compute_text_similarity("这是一个测试假设", "这是一个测试假设")
        assert score1 == 1.0

        # 不同文本
        score2 = self.agent._compute_text_similarity("趋势策略 EMA 交叉", "成交量异常 波动率")
        assert score2 < 1.0
...
class TestRiskCostRedTeamAgent:
    ...
    def test_failure_mode_clarity_check(self):
        ...
        # 模糊失效条件
        vague_proposal = create_test_proposal(failure_modes=["可能失效"])
    ...
    def test_risk_score_calculation(self):
        ...
        # 加权平均
        expected = (
            data_quality * 0.3 + cost_fragility * 0.3 + liquidity * 0.2 + failure_mode_clarity * 0.2
        )
...
class TestReviewerBoundaryConstraints:
    ...
    def test_no_direct_order_in_output(self):
        ...
        # 正常提案
        normal_proposal = create_test_proposal(hypothesis="基于 EMA 的趋势策略")

...
class ResponseScript:
    ...
    def add_429(self, retry_after: int = 60, body: Any = None) -> "ResponseScript":
        """添加 429 响应"""
        return self.add_response(HTTPResponse(429, {"Retry-After": str(retry_after)}, body))
    ...
    def add_timeout(self) -> "ResponseScript":
        ...
        return self.add_response(
            HTTPResponse(504, {}, None, asyncio.TimeoutError("Request timeout"))
        )
...
class FakeHTTPClient:
    ...
    def set_now_fn(
        self, now_fn: Callable[[], float], now_fn_ns: Optional[Callable[[], int]] = None
    ) -> None:
        """注入时间函数"""
        self._now_fn = now_fn

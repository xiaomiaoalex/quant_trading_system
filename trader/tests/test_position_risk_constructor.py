...
@pytest.fixture
def constructor_with_mocks(
    default_config, mock_regime_provider, mock_cooldown_tracker
) -> PositionRiskConstructor:
    """创建带有 mock 的构造函数实例"""
...
class TestCooldown:
    ...
    def test_different_symbol(self, constructor_with_mocks):
        ...
        constructor_with_mocks._cooldown_tracker.get_last_trade_time.side_effect = lambda s: (
            btc_last_trade if s == "BTCUSDT" else None
        )
...
class TestCompleteConstruction:
    ...
    def test_full_construction_fail_on_symbol(
        self, constructor_with_mocks, sample_signal, sample_position
    ):
        """完整流程：单币种暴露超限"""
        # 创建一个已达到最大暴露的持仓
    ...
    def test_full_construction_fail_on_cooldown(
        self, constructor_with_mocks, sample_signal, sample_position
    ):
        """完整流程：冷却期中"""
        # 设置冷却中
...
class TestAdjustedQuantity:
    ...
    def test_calculate_adjusted_qty_pass(
        self, constructor_with_mocks, sample_signal, empty_position
    ):
        """正常计算（空仓情况，不触发仓位占比限制）"""
        constructor_with_mocks._cooldown_tracker.get_last_trade_time.return_value = None
        constructor_with_mocks._regime_provider.get_current_regime.return_value = MarketRegime.BULL
    ...
    def test_calculate_adjusted_qty_fail(
        self, constructor_with_mocks, sample_signal, sample_position
    ):
        """检查失败返回 None"""
        # 设置冷却中
    ...
    def test_calculate_with_crisis_regime(
        self, constructor_with_mocks, sample_signal, empty_position
    ):
        """CRISIS regime 折扣（空仓情况）"""
        constructor_with_mocks._cooldown_tracker.get_last_trade_time.return_value = None
        constructor_with_mocks._regime_provider.get_current_regime.return_value = (
            MarketRegime.CRISIS
        )

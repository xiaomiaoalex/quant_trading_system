...
class MockSnapshotConnection:
    ...
    async def fetch(self, query, *args):
        ...
        if "ORDER BY" in query and "ts_ms DESC" in query:
            ...
            snapshots.sort(
                key=lambda x: x.get("updated_at", x.get("created_at", datetime.min)), reverse=True
            )
...
class TestSnapshotStorageIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_snapshot_persistence_workflow(
        self, pg_connection_string, cleanup_postgres_pool
    ):

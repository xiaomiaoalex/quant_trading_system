...
def test_promote_paper_load_failed_triggers_rollback():
    ...
    updated = storage.get_strategy_candidate(candidate_id)
    assert (
        updated["status"] == "VALIDATION_PASSED"
    ...
    if dep_id:
        ...
        assert (
            dep is None or dep.get("status") == "PROMOTE_ROLLED_BACK"
    ...
    assert any(
        "PROMOTE_LOAD_FAILED" in r or "promote_load_failed" in r for r in event_reasons
...
def test_promote_paper_rollback_marks_deployment():
    ...
    for dep in all_deps:
        if dep.get("status") == "LOADING":
            pytest.fail(f"Deployment {dep['deployment_id']} stuck in LOADING after rollback")
...
def test_promote_paper_approve_failure_triggers_rollback():
    ...
    with TestClient(app) as client:
        with patch(
            ...
            with std_patch.object(
                storage, "update_strategy_candidate", side_effect=update_after_load_fails
            ):
                resp = client.post(f"/v1/strategy-candidates/{candidate_id}/promote-paper")
    ...
    assert (
        dep.get("status") == "PROMOTE_ROLLED_BACK"

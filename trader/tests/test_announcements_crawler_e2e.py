...
class MockStrategy:
    ...
    async def subscribe_to_stream(
        self, event_store: EventStoreWithFallback, stream_key: str = "announcements"
    ):
        """
        订阅事件流（轮询模式）
...
class TestAnnouncementCrawlerE2E:
    ...
    @pytest.mark.asyncio
    async def test_multiple_strategies_subscribe(self, event_store_with_fallback, mock_http_client):
        """
        测试多个策略同时订阅同一事件流

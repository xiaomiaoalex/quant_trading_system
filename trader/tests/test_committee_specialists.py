...
from insight.committee.schemas import SpecialistType, ValidationResult, Violation, ViolationType
...
class TestTrendAgent:
    ...
    def test_do_research_ema_keyword(self):
        """测试 EMA 关键词识别"""
        result = self.agent._do_research("研究 EMA 交叉策略", {})
    ...
    def test_do_research_momentum_keyword(self):
        """测试动量关键词识别"""
        result = self.agent._do_research("研究 momentum 策略", {})
        # 动量策略应该触发价格动量相关特征
    ...
    def test_do_research_bollinger_keyword(self):
        """测试布林带关键词识别"""
        result = self.agent._do_research("研究 Bollinger Band 突破策略", {})
    ...
    def test_do_research_adx_keyword(self):
        """测试 ADX 关键词识别"""
        result = self.agent._do_research("研究趋势强度 ADX 策略", {})
        assert "adx" in result["required_features"]

    def test_do_research_default_types(self):
        """测试默认趋势类型"""
        result = self.agent._do_research("随便研究点什么", {})
        # 默认应该包含 EMA 和动量相关特征
        assert (
            "ema_fast" in result["required_features"] or "price_roc" in result["required_features"]
        )

    def test_hypothesis_contains_strategy_description(self):
        """测试假设包含策略描述"""
        result = self.agent._do_research("研究趋势策略", {})
    ...
    def test_evidence_refs_format(self):
        """测试证据引用格式"""
        result = self.agent._do_research("研究 EMA 策略", {})
...
class TestPriceVolumeAgent:
    ...
    def test_do_research_volume_keyword(self):
        """测试成交量关键词识别"""
        result = self.agent._do_research("研究成交量异常策略", {})
        # 成交量策略应该触发成交量相关特征
        assert "volume_roc" in result["required_features"]
    ...
    def test_do_research_volatility_keyword(self):
        """测试波动率关键词识别"""
        result = self.agent._do_research("研究波动率压缩策略", {})
        # 波动率策略应该触发波动率相关特征
    ...
    def test_do_research_divergence_keyword(self):
        """测试背离关键词识别"""
        result = self.agent._do_research("研究价量背离策略", {})
        # 背离策略应该触发背离相关特征
        assert "price_roc" in result["required_features"]
        assert "volume_roc" in result["required_features"]
    ...
    def test_do_research_open_interest_keyword(self):
        """测试持仓量关键词识别"""
        result = self.agent._do_research("研究持仓量变化策略", {})
        # 持仓量策略应该触发持仓量相关特征
...
class TestFundingOIAgent:
    ...
    def test_do_research_funding_keyword(self):
        """测试资金费率关键词识别"""
        result = self.agent._do_research("研究资金费率异常策略", {})
        # 资金费率策略应该触发资金费率相关特征
    ...
    def test_do_research_oi_keyword(self):
        """测试 OI 关键词识别"""
        result = self.agent._do_research("研究持仓量 OI 策略", {})
        # OI 策略应该触发持仓量相关特征
        assert "open_interest" in result["required_features"]
        assert "open_interest_roc" in result["required_features"]
    ...
    def test_do_research_long_short_keyword(self):
        """测试多空比关键词识别"""
        result = self.agent._do_research("研究多空比策略", {})
        # 多空比策略应该触发多空比相关特征
...
class TestOnChainAgent:
    ...
    def test_do_research_exchange_keyword(self):
        """测试交易所关键词识别"""
        result = self.agent._do_research("研究交易所净流量策略", {})
        # 交易所净流量策略应该触发相关特征
    ...
    def test_do_research_liquidation_keyword(self):
        """测试爆仓关键词识别"""
        result = self.agent._do_research("研究爆仓数据策略", {})
        # 爆仓策略应该触发爆仓相关特征
    ...
    def test_do_research_holder_keyword(self):
        """测试持币者关键词识别"""
        result = self.agent._do_research("研究持币地址策略", {})
        # 持币地址策略应该触发相关特征
...
class TestEventRegimeAgent:
    ...
    def test_do_research_announcement_keyword(self):
        """测试公告关键词识别"""
        result = self.agent._do_research("研究公告事件策略", {})
        # 公告事件策略应该触发相关特征
    ...
    def test_do_research_regime_keyword(self):
        """测试状态机关键词识别"""
        result = self.agent._do_research("研究市场状态转换策略", {})
        # 状态转换策略应该触发相关特征
    ...
    def test_do_research_macro_keyword(self):
        """测试宏观事件关键词识别"""
        result = self.agent._do_research("研究宏观事件策略", {})
        # 宏观事件策略应该触发相关特征
...
class TestCommitteeRouter:
    ...
    def test_run_specialists(self):
        """测试运行 specialists"""
        outputs = self.router.run_specialists("研究 EMA 策略", {SpecialistType.TREND}, {})

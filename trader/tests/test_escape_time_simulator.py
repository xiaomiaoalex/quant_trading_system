...
def create_orderbook(
    ...
    return OrderBook(
        symbol="BTCUSDT",
        bids=[
            OrderBookLevel(price=Decimal(str(p)), quantity=Decimal(str(q))) for _, p, q in bids_list
        ],
        asks=[
            OrderBookLevel(price=Decimal(str(p)), quantity=Decimal(str(q))) for _, p, q in asks_list
        ],
...
class TestKillSwitchBlocking:
    ...
    def test_l3_blocks_escape(self):
        """L3 级别应阻止平仓"""
        mock_killswitch = Mock()
        mock_killswitch.get_killswitch_level.return_value = (
            KillSwitchLevel.L3_LIQUIDATE_AND_DISCONNECT
        )
...
class TestCooldownBlocking:
    ...
    def test_in_cooldown_blocks_escape(self):
        ...
        mock_cooldown.get_last_trade_time.return_value = datetime.now(timezone.utc) - timedelta(
            seconds=100
        )
...
class TestDepthSimulation:
    ...
    def test_escape_path_contains_levels(self):
        ...
        simulator = EscapeTimeSimulator(risk_config=create_risk_config())
        position = create_position(quantity=1.0)
        orderbook = create_orderbook(bids=[("BTCUSDT", 50000.0, 10.0), ("BTCUSDT", 49900.0, 20.0)])
    ...
    def test_short_position_slippage_with_asks(self):
        ...
        # 买盘为空，卖盘价格高于中间价
        orderbook = create_orderbook(
            bids=[], asks=[("BTCUSDT", 50100.0, 15.0), ("BTCUSDT", 50200.0, 25.0)]  # 买盘为空
        )
    ...
    def test_insufficient_depth_blocks_escape(self):
        ...
        orderbook = create_orderbook(bids=[("BTCUSDT", 50000.0, 0.1)])  # 只有 0.1 BTC 深度，远不足
...
class TestRegimeDiscount:
    ...
    def test_crisis_reduces_escapable_quantity(self):
        ...
        orderbook = create_orderbook(bids=[("BTCUSDT", 50000.0, 0.5)])
...
class TestDepthLevel:
    ...
    def test_depth_level_immutable(self):
        """DepthLevel 应该是不可变的"""
        level = DepthLevel(
            price=Decimal("50000"), cumulative_quantity=Decimal("1.0"), level_index=0
        )
    ...
    def test_depth_level_attributes(self):
        """DepthLevel 属性应正确"""
        level = DepthLevel(
            price=Decimal("50000"), cumulative_quantity=Decimal("1.0"), level_index=5
        )

...
class SleeveData:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> SleeveData:
        return cls(
            ...
            capital_cap=(
                Decimal(data["capital_cap"])
                if isinstance(data["capital_cap"], str)
                else Decimal(str(data["capital_cap"]))
            ),
            weight=data.get("weight", 0.0),
            max_position_size=(
                Decimal(data["max_position_size"])
                if isinstance(data["max_position_size"], str)
                else Decimal(str(data.get("max_position_size", "0")))
            ),
...
class ProposalModel:
    ...
    def create_portfolio(
        ...
        **kwargs,
        ...
        return cls(proposal_type=ProposalType.PORTFOLIO, payload=payload, **kwargs)

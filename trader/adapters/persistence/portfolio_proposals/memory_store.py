...
class InMemoryPortfolioProposalStore:
    ...
    async def list_by_status(
        ...
        with self._lock:
            filtered = [p for p in self._proposals.values() if p.status == status]
    ...
    async def list_by_type(
        ...
        with self._lock:
            filtered = [p for p in self._proposals.values() if p.proposal_type == proposal_type]
            # 按 created_at DESC 排序
            filtered.sort(key=lambda p: p.created_at, reverse=True)
            return filtered[offset : offset + limit]
    ...
    async def list_by_specialist(
        ...
        with self._lock:
            filtered = [
                ...
                if p.proposal_type == ProposalType.SLEEVE and p.specialist_type == specialist_type
            ]
            # 按 created_at DESC 排序
            filtered.sort(key=lambda p: p.created_at, reverse=True)
            return filtered[offset : offset + limit]

    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_check_at": (
                ...
                if isinstance(self.last_check_at, datetime)
                else self.last_check_at
            ),
            ...
            "last_rejection_at": (
                self.last_rejection_at.isoformat()
                if isinstance(self.last_rejection_at, datetime)
                else self.last_rejection_at
            ),
...
def classify_rejection_reason(reason: str) -> str:
    ...
    if "drawdown" in reason_lower or "max loss" in reason_lower or "daily loss" in reason_lower:
        ...
        "position limit" in reason_lower
        or "position size" in reason_lower
        or "max position" in reason_lower
    ):

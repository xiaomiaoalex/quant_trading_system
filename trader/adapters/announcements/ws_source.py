...
class BinanceWsAnnouncementSource:
                    ...
                    try:
                        ...
                        if msg_type == "DATA":
                            ...
                            if ann.title or ann.body:
                                logger.debug(
                                    "WS_SOURCE_RECEIVED", extra={"title": (ann.title or "")[:40]}
                                )
                                return ann
                            else:
                                # 空消息，记录 debug 并计数
                ...
                try:
                    ...
                    if msg_type == "DATA":
                        ...
                        if ann.title or ann.body:
                            ...
                            logger.debug(
                                "WS_SOURCE_EMPTY_MESSAGE_SKIPPED", extra={"skip_count": skip_count}
                            )
                            continue
    ...
    async def fetch_initial(self, max_results: int = 100) -> list[RawAnnouncement]:
        ...
        warnings.warn(
            "WS source does not support fetch_initial. " "Use HTML source for bootstrap/backfill.",
            DeprecationWarning,
            stacklevel=2,
        )
        return []

...
本文档用于定义 `quant_trading_system Crypto v3.3.0` 的工程实现优先级，回答一个最关键的问题：
...
### 备注
v3.3.0 中，Reconciler 不再只是“以后再说”的配件，而是主闭环的必备守门员。
...
Crypto v3.3.0 的实现优先级核心，不是先让系统“做更多事”，而是先让系统 **拥有足够深的数据视野、足够硬的生存纪律、足够清楚的复用边界**；只有先把这些做对，后面的策略和 AI 才值得接进去。
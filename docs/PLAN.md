...
## 后续 Crypto Risk 开发停顿与审计交接要求

自 P4.5 起，后续每完成一个独立开发段落（如 P4.5、P4.6、P4.7、P5、P6、P7、P8、P9），AI 开发工程师必须立即停下来，不得自行连续推进下一段。停顿时必须输出审计交接包，供主审对照代码库变动进行审计。

审计交接包必须包含：

1. 本段目标与实际完成范围：明确哪些子任务完成，哪些未做，哪些被推迟。
2. 代码变更清单：按 Core / Adapter / Service / API / Persistence / Frontend / Tests / Docs 分类列出文件。
3. 接口与契约变更：说明是否修改 DTO、API schema、事件 payload、环境变量、持久化字段；如有，必须引用 `docs/INTERFACE_CONTRACTS.md` 中对应更新。
4. 架构边界说明：说明是否影响层级边界、IO 位置、风控闭环、审计链路或运行拓扑；如有，必须引用 `docs/PROJECT_ARCHITECTURE.md` 中对应更新。
5. 风控行为变化：列出新增/修改的 rejection reason、KillSwitch 推荐级别、fail-closed 场景、close-only/裁剪行为。
6. 测试与验证：列出完整命令、结果、失败后修复情况；未运行的测试必须说明原因。
7. 风险与遗留：列出已知缺口、兼容性风险、生产联调注意事项和下一段建议入口。
8. Git 信息：列出当前分支、commit hash、工作区是否 clean；若存在用户未提交改动，必须明确未触碰范围。
...
| Stage 5 | CapitalAllocator OMS 接入 | ✅ 已完成（含审查返工 + 2026-06-07 预算模式升级） | `StrategyRunner` OMS 前置 allocation gate、热配置、trace 审计、profile 更新审计、绝对金额/NAV比例预算、portfolio-wide reservation lock、OMS 成功后提交 committed notional |
...
> 注：本文件历史阶段计划较长。Qlib/Hermes 研究编排主线已收敛为 Research Layer，
> P8 已完成 Demo fail-closed 演练。自 2026-05-14 起，当前执行主线切换为
> “P9 — 跨市场扩展准备 + EventDrivenRiskReplay 设计准备”。
> 历史 Phase 计划保留为背景记录；任务排序以本节和最近开发记录优先。
...
- **P9**: 跨市场扩展准备 + EventDrivenRiskReplay 设计准备（P9.0/P9.1/P9.2/P9.3/P9.4 已完成，等待 P9.5）
...
## 已完成研究主线：Phase 8 — v3.4.0 Qlib + Hermes 研究编排集成

## 当前执行主线：P9 — 跨市场扩展准备 + EventDrivenRiskReplay

### P9 设计原则

P9 采用“市场无关规则接口 + 市场专用规则插件”架构：

1. 市场无关层只定义 pre-trade rule contract、插件接口、结果格式和 fail-closed 语义。
2. A 股规则不进入通用层，全部放入 `ChinaStockMarketRulePlugin`。
3. Crypto 规则作为 crypto specialization，优先包装已有 `ExchangeRuleGuard`。
4. `EventDrivenRiskReplay` 复用 `RiskEngine.check_pre_trade()`，不复制风控逻辑。
...
Qlib/Hermes 只作为 Research/Insight 层：产出因子、模型、预测和研究报告；Qlib 输出必须通过 `QlibToStrategyBridge` 转为内部 `Signal`，再进入 `RiskEngine.check_pre_trade()`。当前 active backtest path 是 VectorBT / `VectorBTAdapterWithRisk`；生产级订单、账户、风控、OMS 回放能力归入后续 `EventDrivenRiskReplay` 设计准备。
...
| VectorBT / VectorBTAdapterWithRisk（当前 active 回测路径） | 没有要求每个策略必须回答"机制假设" |

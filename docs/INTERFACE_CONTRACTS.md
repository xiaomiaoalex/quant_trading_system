...
### 8.5.2 Funding/OI Runtime Contract（阶段3生产数据接线）

**数据源接口**：

| Protocol | 位置 | 职责 |
|----------|------|------|
| `FundingOIHistoryPort` | `trader/services/funding_oi_metrics_provider.py` | 从 FeatureStore 读取历史 Funding Rate 和 OI 数据 |
| `CurrentFundingOIPort` | `trader/services/funding_oi_metrics_provider.py` | 获取当前 Funding Rate 和 OI 及最新时间戳 |
| `FundingOIMetricsProvider` | Service | 聚合历史 + 当前数据，调用 Core 计算器 |

**数据源契约**：

| 数据源 | 接口 | Freshness 要求 | 失败处理 |
|--------|------|----------------|----------|
| 历史 Funding Rate | `FundingOIHistoryPort.read_funding_history()` | 必须覆盖 `funding_history_window` 配置的天数 | 数据不足时返回空列表，设置 `funding_window_insufficient=True` |
| 历史 OI | `FundingOIHistoryPort.read_oi_history()` | 必须覆盖 `oi_history_window` 配置的天数 | 数据不足时返回空列表，设置 `oi_window_insufficient=True` |
| 当前 Funding Rate | `CurrentFundingOIPort.get_current_funding_rate()` | 必须返回最新 Funding Rate | 缺失时返回 `None`，设置 `funding_current_missing=True` |
| 当前 OI | `CurrentFundingOIPort.get_current_open_interest()` | 必须返回最新 OI | 缺失时返回 `None`，设置 `oi_current_missing=True` |
| 最新 Funding 时间戳 | `CurrentFundingOIPort.get_latest_funding_ts_ms()` | 必须返回数据时间戳 | 缺失时返回 `0` |
| 最新 OI 时间戳 | `CurrentFundingOIPort.get_latest_oi_ts_ms()` | 必须返回数据时间戳 | 缺失时返回 `0` |

**Freshness 要求**：

| 场景 | 判断标准 | 处理行为 |
|------|----------|----------|
| Funding 数据新鲜 | `current_ts_ms - latest_funding_ts_ms <= max_data_age_seconds` | `funding_data_stale=False` |
| Funding 数据过期 | `current_ts_ms - latest_funding_ts_ms > max_data_age_seconds` | `funding_data_stale=True` |
| OI 数据新鲜 | `current_ts_ms - latest_oi_ts_ms <= max_data_age_seconds` | `oi_data_stale=False` |
| OI 数据过期 | `current_ts_ms - latest_oi_ts_ms > max_data_age_seconds` | `oi_data_stale=True` |
| Funding 窗口充足 | `len(funding_history) >= funding_min_periods` | 可计算 Z-Score |
| Funding 窗口不足 | `len(funding_history) < funding_min_periods` | `funding_rate_z_score=None`，`funding_window_insufficient=True` |
| OI 窗口充足 | `len(oi_history) >= oi_min_periods` | 可计算变化率 |
| OI 窗口不足 | `len(oi_history) < oi_min_periods` | `open_interest_change_rate=None`，`oi_window_insufficient=True` |

**fail-closed 行为矩阵**：

| 阈值状态 | 数据状态 | 指标状态 | 风控行为 |
|----------|----------|----------|----------|
| funding 阈值**启用** | funding 数据**过期** | - | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| funding 阈值**启用** | funding **窗口不足** | - | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| funding 阈值**启用** | funding **当前值缺失** | - | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| funding 阈值**启用** | - | `funding_rate_z_score` **超过阈值** | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| funding 阈值**禁用** | funding 数据缺失/过期 | - | **通过**（不检查 funding） |
| OI 阈值**启用** | OI 数据**过期** | - | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| OI 阈值**启用** | OI **窗口不足** | - | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| OI 阈值**启用** | OI **当前值缺失** | - | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| OI 阈值**启用** | - | `open_interest_change_rate` **超过阈值** | **REJECT** + `CRYPTO_FUNDING_OI_RISK` |
| OI 阈值**禁用** | OI 数据缺失/过期 | - | **通过**（不检查 OI） |

**关键约束**：
- funding 和 OI **独立计算**，互不影响：
  - 缺 funding 不影响 OI 指标检查
  - 缺 OI 不影响 funding 指标检查
- `current_funding_rate=None` 和 `current_open_interest=None` 不得转成 `0.0` 制造虚假 Z-Score。
- `open_interest_change_rate` 是百分比变化率 `(current_oi - mean) / mean * 100`，不是 Z-Score。
- Adapter/Service 层读取数据，Core 层只消费 snapshot。
- 单元测试不得访问真实网络，必须用 fake provider。

**阈值启用判断**：
```
funding_threshold_enabled = budget.max_abs_funding_rate_z_score is not None and budget.max_abs_funding_rate_z_score > 0
oi_threshold_enabled = budget.max_abs_open_interest_change_rate is not None and budget.max_abs_open_interest_change_rate > 0
```
...
#### 8.7.5 RiskSizingDecision 决策语义与 fail-closed 约束

**决策类型语义**：

| Decision | Broker 收到订单 | OMS 使用数量 | 审计要求 |
|----------|----------------|--------------|----------|
| `APPROVE` | 是 | `requested_qty`（或 `normalized_qty`） | 记录 requested_qty |
| `CLIP` | 是 | **`final_qty`**（必须） | 记录 `requested_qty` 和 `final_qty` |
| `REJECT` | **否** | 不适用 | 记录 `requested_qty` 和 `rejection_reason` |
| `CLOSE_ONLY` | **否**（仅减仓信号例外） | 仅当 `is_close_signal=True` 时允许 `final_qty` | 记录信号类型和拒绝原因 |

**CLIP 语义澄清（关键）**：
- `CLIP` 不再被当作普通 reject。它代表"允许下单，但**必须**使用 `final_qty`"。
- `REJECT` 才代表 broker 不得收到任何订单。
- `final_qty` 必须通过 `max_allowed_qty` 计算，不得由 OMS 重新计算。
- OMS 不允许绕过 `final_qty` 使用原始 `requested_qty`。

**fail-closed 约束**：
- `final_qty <= 0` 或 `final_qty is None` 时必须返回 `REJECT`，不得返回 `CLIP`。
- `normalized_qty <= 0` 时必须返回 `REJECT`，不得返回 `CLIP`。
- 缺少关键风控数据时（mark price、instrument spec、account data）必须返回 `REJECT`，不得补 0 或猜默认值。
- `except: pass` 静默通过是禁止的。

**迁移期兼容**：
- 旧实现可能使用 `passed=False + risk_sizing_decision` 表示裁剪。
- 新实现必须产出新语义：`CLIP` = `passed=True` + `decision=CLIP` + `final_qty < requested_qty`。
- 迁移期内测试必须覆盖两种语义路径，确保向后兼容。

**live 与 backtest 一致性**：
- `RiskSizingEngine.calculate(signal, snapshot, trace_id)` 是纯计算入口，输出 `RiskSizingDecision`。
- live OMS 和 backtest RiskEngine 必须共用 `RiskSizingEngine.calculate(...)` 的结果。
- 禁止在 OMS 层重新实现 sizing 数学，只能消费核心层决策。
...
CLIP 订单必须：
- 在审计中同时记录 `requested_qty` 和 `final_qty`
- broker 实际收到 `final_qty`，不得是原始数量
...
**NO_NEW_POSITIONS (L1)**：
- 禁止新开净多头/空头（`allows_new_positions=False`）
- 允许减仓（平仓现有持仓）
- 允许取消现有挂单
- StrategyRunner/OMS 必须拦截任何可能导致净敞口增加的订单

**CLOSE_ONLY (L2)**：
- 禁止任何新开仓或加仓
- 只允许 `reduce_only=True` 的订单（减仓/平仓）
- 允许撤销现有挂单（不影响 `reduce_only` 限制）
- 拒绝时应返回 `RiskCheckResult.passed=False` + `rejection_reason=CLOSE_ONLY`

**CANCEL_ALL_AND_HALT (L3)**：
- **必须**先执行 `cancel_all`，撤销所有未成交挂单
- 禁止所有新订单（包括减仓）
- broker 不得收到任何 `place_order` 调用
- 系统强平 actor 可以发出 `reduce_only` 强平单

**LIQUIDATE_AND_DISCONNECT (L3+)**：
- 禁止策略订单（任何 `place_order` 调用）
- 禁止手动撤单
...
- 同一 session 内模式只能升级，不能静默降级

**与 KillSwitch 集成**：
- `NO_NEW_POSITIONS` 对应 `KillSwitchLevel.L1_NO_NEW_POSITIONS`
- `CLOSE_ONLY` 对应 `KillSwitchLevel.L2_CLOSE_ONLY`
- `CANCEL_ALL_AND_HALT` 对应 `KillSwitchLevel.L3_FULL_STOP`
- `LIQUIDATE_AND_DISCONNECT` 对应 `KillSwitchLevel.L3_FULL_STOP` + 强平标志
- KillSwitch 推荐级别高于当前 RiskMode 时，必须执行升级

**fail-closed 约束**：
- 当 `recommended_killswitch_level > current_level` 时，必须升级
- RiskMode 返回不明确的权限时，默认 fail-closed（不允许操作）
- 缺少 RiskMode 状态数据时，默认进入 `NO_NEW_POSITIONS`
...
- `CANCEL_ALL_AND_HALT` 及以上拦截所有策略订单；系统强平 actor 的 `reduce_only liquidation` 例外

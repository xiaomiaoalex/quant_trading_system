...
**问题描述**：  
`runtime_state_storage` 需要在 `StrategyRunner` 创建时就传入，但 `get_strategy_runner()` 是延迟初始化（首次调用才创建单例）。如果 storage 本身也依赖延迟初始化的组件，可能出现循环依赖。

**解决方案**：  
- `get_strategy_runner()` 内部直接调用 `get_storage()` 获取 `ControlPlaneInMemoryStorage` 单例
- `StrategyRunner` 只负责保存引用，存储的创建时机由 `get_storage()` 控制
- 这保证了存储和运行器的生命周期一致

**经验**：  
- 依赖注入应该从"获取点"而非"创建点"处理延迟初始化
- 单例模式的延迟初始化要小心循环引用

---

### 0.1 asyncio.Lock 在策略级别需要按 strategy_id 隔离

**问题描述**：  
多策略并发时，如果使用全局锁会降低并发性能；如果完全不加锁，策略间的状态更新会互相干扰。

**解决方案**：  
- 在 `StrategyRunner` 中使用 `Dict[str, asyncio.Lock]` 按 strategy_id 创建锁
- `_get_strategy_lock(strategy_id)` 方法在首次访问某策略时创建锁
- 这样每策略独立锁，不阻塞其他策略

**经验**：  
- 并发设计要识别"策略内"和"策略间"的边界
- 按 ID 分桶的锁粒度是平衡性能和安全的合理选择
...
**问题描述**：  
如果每次 tick 都持久化状态，会产生大量 IO；策略启动/停止频繁调用存储也会影响性能。

**解决方案**：  
- `start()` 和 `stop()` 时立即持久化（状态变更点）
- `tick()` 中每 60 个 tick 才持久化一次（使用 `tick_count % 60 == 0`）
- 使用 `asyncio.create_task()` 在后台异步执行恢复，不阻塞启动流程

**经验**：  
- 持久化策略应该是"状态变更点触发 + 周期采样"的组合
- 后台任务的错误要用 try/except 包裹，避免静默失败

---

### 0.3 恢复逻辑中的环境一致性检查不能跳过

**问题描述**：  
策略在不同环境（demo/testnet/live）的配置和状态不能混用，如果直接恢复可能产生不可预期的交易行为。

**解决方案**：  
- 恢复时必须比较 `saved_env` 和当前 `binance_env`
- 不一致时记录 `recovery_error` 并跳过恢复，而不是强制恢复
- 日志要清晰标注 `env_mismatch` 便于排查

**经验**：  
- 交易系统的环境隔离是硬约束，恢复逻辑必须验证
- 跳过恢复时要用 `recovery_error` 记录原因，不能静默忽略
...
## 十八、Binance `-1021` 时间偏移修复经验（签名请求）

### 18.1 踩坑记录：仅调用 `/v3/time` 但不参与签名时间，不等于完成时间同步

**场景**：
Broker 在连接时已经调用过 `GET /v3/time`，但签名请求仍直接用 `int(time.time() * 1000)` 作为 `timestamp`。

**问题**：
- 本机时钟只要比交易所快约 1 秒，就会触发 `code=-1021`
- 典型症状是 `GET /v3/account` 失败，进一步导致 Reconciler 的交易所对账抓取为空

**经验**：
- “有时间接口调用”不等于“已完成时间对齐”
- 必须把服务端时间换算出的偏移量真正用于签名请求构造

### 18.2 设计模式：偏移缓存 + 错误触发重校准 + 单次重试

**实现模式**：
1. 启动连接时通过 `/v3/time` 计算 `time_offset_ms = server_time_ms - local_time_ms`
2. 所有 signed 请求使用 `local_time + time_offset_ms` 生成 `timestamp`
3. 若返回 `-1021`，立即刷新偏移并对同一请求重签名重试一次

**收益**：
- 兼容本机时钟轻微漂移
- 不需要依赖手工校时才能稳定启动
- 保持 Fail-Closed：重试后仍失败时继续抛出业务异常，不吞错

### 18.3 测试护栏：把“签名前时间构造”变成可回归验证

新增 `trader/tests/test_binance_spot_demo_broker.py`，覆盖：
- 偏移量计算正确（server-local）
- signed 请求使用偏移后时间戳
- 收到 `-1021` 后会重校准并重试成功

这类测试可以在不访问真实网络的前提下，稳定复现过去线上才会暴露的时间偏移故障。

---

## 十九、Binance `recvWindow` 环境配置经验（控制面接线）

### 19.1 踩坑记录：单入口改配置会造成“同功能不同表现”

**场景**：
系统同时存在两个会拉交易所订单的入口：
1. 应用启动后的周期性对账（`trader/api/main.py`）
2. 手动触发对账（`POST /v1/reconciler/trigger`）

**问题**：
- 若只在一个入口接 `recvWindow` 配置，另一个入口仍用默认值
- 现场表现会变成“后台周期对账正常，但手动触发偶发失败”（或反过来）

**经验**：
- 交易所连接参数属于“跨入口共享配置”，必须统一收口解析并双入口同步接线

### 19.2 设计模式：配置收口模块 + 边界值策略

**实现模式**：
- 新增 `trader/api/env_config.py` 统一解析 `BINANCE_RECV_WINDOW`
- 规则：
  - 未配置/空值：默认 `5000`
  - 非法值或小于 1：回退默认 `5000`
  - 超过上限：夹紧到 `60000`
...
**收益**：
- 避免散落式 `os.environ.get(...)` 带来的逻辑漂移
- 让问题定位从“代码路径排查”收敛成“环境变量与边界规则校验”

### 19.3 额外修复：配置工厂调用必须与实际 Adapter API 一致

**场景**：
`reconciler` 路由里曾调用 `BinanceSpotDemoBrokerConfig.create(...)`，但适配器实际暴露的是 `for_demo(...) / for_testnet(...)`。
...
## 二十五、订单归属注册表实现经验（自动化归属识别）

### 25.1 踩坑记录：前缀过滤无法覆盖"已关闭/废弃策略"的历史订单

**场景**：
用户通过 `RECONCILER_EXCHANGE_CLIENT_ORDER_PREFIXES` 设置前缀过滤，但每次新增/删除策略都需要手动更新环境变量。

**问题**：
- 历史订单的 `client_order_id` 可能不带有当前策略前缀
- 无法自动识别"本系统历史订单"vs"外部历史订单"

**经验**：
- 需要一个中心化的归属注册表，自动追踪订单来源
- 支持启动时从本地订单/事件回填注册表

### 25.2 设计模式：订单归属注册表（OrderOwnershipRegistry）
...
class OrderOwnershipRegistry:
    ...
    def classify_order(self, client_order_id: str) -> OrderOwnership:
        """
        分类结果：
        - OWNED: 本系统订单，参与对账
        ...
        """
        # 1. 已注册订单 -> 基于注册判断
        # 2. 命名空间前缀（如 QTS1_）-> OWNED
        # 3. 外部前缀列表 -> EXTERNAL
        # 4. 其他 -> UNKNOWN（保守，不触发告警）
```

**收益**：
- 自动化订单归属识别，无需手动维护前缀列表
- 支持历史订单回填（覆盖已停用的策略）
- 三级分类提供细粒度控制

### 25.3 隐蔽兼容性问题：`external_order_ids=None` 触发旧行为

**场景**：
Reconciler 的 `reconcile()` 方法增加了 `external_order_ids` 参数。

**问题**：
- 如果传入 `None`，表示不启用归属过滤（旧行为兼容）
- 如果传入空集合 `set()`，表示启用但没有外部订单
- 这两种情况语义不同，必须严格区分

**经验**：
- 可选参数使用 `Optional[T]` 并在文档中明确区分 `None` vs 空集合的语义
- 测试必须覆盖这两种边界情况

### 25.4 架构约束：Core Plane 无 IO

**场景**：
`OrderOwnershipRegistry` 位于 `trader/core/domain/services/` 目录下。
